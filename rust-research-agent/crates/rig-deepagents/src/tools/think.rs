...
mod tests {
    ...
    async fn test_think_tool_execute() {
        ...
        assert!(result.contains("Reflection recorded"));
        assert!(result.contains(&format!("{} chars", reflection.len())));
    }
    ...
    async fn test_think_tool_long_reflection() {
        ...
        assert!(!result.contains(&long_thought)); // Should NOT echo the full content
    }
